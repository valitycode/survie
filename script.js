...
const dom = {
  ...
  mobileActionCluster: document.getElementById('mobileActionCluster'),
  ...
  skillCooldownText: document.getElementById('skillCooldownText')
};
...
function syncSettingsUI() {
  if (isTouchPreferredDevice() && settings.controlMode !== 'keyboard') {
    settings.controlMode = 'joystick';
  }
  ...
  dom.mobileActionCluster?.classList.toggle('hidden-visibility', settings.controlMode !== 'joystick' && !isTouchPreferredDevice());
}
...
class InputManager {
  ...
  bind() {
    ...
    dom.joystickArea.addEventListener('pointerdown', startPointer, { passive: false });
    window.addEventListener('pointermove', movePointer, { passive: false });
    window.addEventListener('pointerup', endPointer);
    window.addEventListener('pointercancel', endPointer);

    const bindActionButton = (el, actionName) => {
      if (!el) return;
      const setPressed = (value) => {
        if (actionName === 'dash') this.dashPressed = value;
        if (actionName === 'skill') this.skillPressed = value;
    ...
    bindActionButton(dom.dashButton, 'dash');
    bindActionButton(dom.skillButton, 'skill');
  ...
  consumeDashPressed() {
    const active = this.dashPressed || this.isPressed('shift');
    this.dashPressed = false;
    return active;
  }

  consumeSkillPressed() {
    const active = this.skillPressed || this.isPressed('e') || this.isPressed(' ');
    this.skillPressed = false;
    return active;
  }
}
...
class Player extends Entity {
  ...
  update(dt, game) {
    ...
    this.skillCooldownRemaining = Math.max(0, this.skillCooldownRemaining - dt);
    this.dashTimer = Math.max(0, this.dashTimer - dt);

    if (game.input.consumeDashPressed()) {
      this.tryDash(game);
    }
    if (game.input.consumeSkillPressed()) {
      this.tryUseSkill(game);
    }
    ...
    if (this.dashTimer <= 0) {
      ...
      this.invuln = Math.max(this.invuln, CONFIG.player.dashInvuln);
    }
  ...
  tryDash(game) {
    if (this.dashCooldownRemaining > 0 || game.state !== 'running') return false;
    let dir = game.input.getMoveVector();
    if (Math.abs(dir.x) < 0.1 && Math.abs(dir.y) < 0.1) {
      dir = { x: Math.cos(this.angle), y: Math.sin(this.angle) };
    }
    const n = normalize(dir.x, dir.y);
    this.vx = n.x * CONFIG.player.dashSpeed;
    this.vy = n.y * CONFIG.player.dashSpeed;
    this.dashTimer = CONFIG.player.dashDuration;
    this.dashCooldownRemaining = CONFIG.player.dashCooldown;
    game.camera.shake(4);
    game.spawnRing(this.x, this.y, 42, 'rgba(125,255,176,0.65)');
  ...
  tryUseSkill(game) {
    if (this.skillCooldownRemaining > 0 || game.state !== 'running') return false;
    this.skillCooldownRemaining = CONFIG.player.skillCooldown;
    const radius = CONFIG.player.skillRadius + this.stats.projectileCount * 8;
    const damage = CONFIG.player.skillDamage + this.stats.damage * 0.9;
    let hits = 0;
    for (const enemy of game.enemies) {
      if (enemy.dead) continue;
      ...
      if (dist <= radius + enemy.radius) {
        hits += 1;
        enemy.takeDamage(damage, game, enemy.x, enemy.y, true);
      }
    }
    game.spawnRing(this.x, this.y, radius, 'rgba(110,240,255,0.88)');
    game.camera.shake(7);
    game.camera.triggerFlash();
    for (let i = 0; i < 26; i += 1) {
      const a = (Math.PI * 2 * i) / 26;
      game.particles.push(new Particle(this.x + Math.cos(a) * 24, this.y + Math.sin(a) * 24, {
        vx: Math.cos(a) * rand(120, 280),
        vy: Math.sin(a) * rand(120, 280),
        radius: rand(2, 6),
        life: rand(0.26, 0.52),
        color: 'rgba(110,240,255,0.9)'
      }));
    }
    game.floatingTexts.push(new FloatingText(this.x, this.y - 40, hits > 0 ? `Pulse ${hits} cibles` : 'Pulse', '#7dedff', 18));
    return true;
  }
...
class Game {
  ...
  resize() {
    ...
    const cssWidth = Math.max(1, Math.round(rect.width || this.canvas.clientWidth || window.innerWidth || 1));
    const cssHeight = Math.max(1, Math.round(rect.height || this.canvas.clientHeight || window.innerHeight || 1));
  ...
  scheduleReliableResize() {
    this.resize();
    requestAnimationFrame(() => this.resize());
    setTimeout(() => this.resize(), 40);
    setTimeout(() => this.resize(), 140);
  }
  ...
  startRun() {
    ...
    this.scheduleReliableResize();
    this.input.resizeJoystickMetrics();
  ...
  backToMenu() {
    ...
    this.hideOverlay(dom.pauseOverlay);
    this.hideOverlay(dom.settingsOverlay);
    if (dom.dashCooldownText) dom.dashCooldownText.textContent = '';
    if (dom.skillCooldownText) dom.skillCooldownText.textContent = '';
    dom.dashButton?.classList.remove('is-cooling');
    dom.skillButton?.classList.remove('is-cooling');
  }
  ...
  updateActionButtons() {
    if (!dom.dashButton || !dom.skillButton || !this.player) return;
    const setState = (button, label, remaining) => {
      const cooling = remaining > 0.05;
      button.classList.toggle('is-cooling', cooling);
      label.textContent = cooling ? `${remaining.toFixed(1)}s` : '';
    };
    setState(dom.dashButton, dom.dashCooldownText, this.player.dashCooldownRemaining);
    setState(dom.skillButton, dom.skillCooldownText, this.player.skillCooldownRemaining);
  }
  ...
  updateHelperBadge() {
    ...
    if (settings.controlMode === 'keyboard') {
      ...
      this.helperBadge.innerHTML = 'Déplace le joystick à gauche. DASH pour esquiver, SKILL pour déclencher l’onde.';
    }
  }
...
syncSettingsUI();
const game = new Game();
game.scheduleReliableResize();
game.updateHelperBadge();


// Joystick visible et prioritaire sur appareil tactile.
